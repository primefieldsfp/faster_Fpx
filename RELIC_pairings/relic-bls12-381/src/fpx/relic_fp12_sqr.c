...
/////////////////////////////////////////////////////// Copy of basic fp12_sqr version
void fp12_sqr_lazyr(fp12_t c, fp12_t a) {
	fp6_t t0, t1;
...
//////////////////////////////////////////////////////// Attempt with fp4, but it's slower than fp6 version
/////////////////////////////////////////////////////// Adapted from of fp12_sqr_lazyr version
#if 0
void fp12_sqr_lazyr(fp12_t c, fp12_t a) {
	fp4_t t0, t1;
	fp4_t u0, u1, u2, u3, u4;
	...
	RLC_TRY {
		...
		fp2_copy(t1[1], a[1][2]);
		fp4_mul(u1, t0, t1);
		fp2_add(u1[0], u1[0], u1[0]);
		fp2_add(u1[1], u1[1], u1[1]);

		/* (t4,t5) = a2^2. */
		fp4_sqr(u2, t1);

		/* c2 = a0 + a2. */
		fp2_add(t1[0], a[0][0], a[0][1]);
		fp2_add(t1[1], a[1][1], a[1][2]);

		/* (t6,t7) = (a0 + a2 + a1)^2. */
		fp2_add(t0[0], t1[0], a[1][0]);
		fp2_add(t0[1], t1[1], a[0][2]);
		fp4_sqr(u3, t0);

		/* c2 = (a0 + a2 - a1)^2. */
		fp2_sub(t0[0], t1[0], a[1][0]);
		fp2_sub(t0[1], t1[1], a[0][2]);
		fp4_sqr(u4, t0);

		/* c2 = (c2 + (t6,t7))/2. */
		fp2_add(u4[0], u4[0], u3[0]);
		fp2_add(u4[1], u4[1], u3[1]);
		fp_hlv(u4[0][0], u4[0][0]);
		fp_hlv(u4[0][1], u4[0][1]);
		fp_hlv(u4[1][0], u4[1][0]);
		fp_hlv(u4[1][1], u4[1][1]);

		/* (t6,t7) = (t6,t7) - c2 - (t2,t3). */
		fp2_sub(u3[0], u3[0], u4[0]);
		fp2_sub(u3[1], u3[1], u4[1]);
		fp2_sub(u3[0], u3[0], u1[0]);
		fp2_sub(u3[1], u3[1], u1[1]);

		/* c2 = c2 - (t0,t1) - (t4,t5). */
		fp2_sub(u4[0], u4[0], u0[0]);
		fp2_sub(u4[1], u4[1], u0[1]);
		fp2_sub(c[0][1], u4[0], u2[0]);
		fp2_sub(c[1][2], u4[1], u2[1]);

		/* c1 = (t6,t7) + (t4,t5) * E. */
		fp2_mul_nor(u4[1], u2[1]);
		fp2_add(c[1][0], u3[0], u4[1]);
		fp2_add(c[0][2], u3[1], u2[0]);

		/* c0 = (t0,t1) + (t2,t3) * E. */
		fp2_mul_nor(u4[1], u1[1]);
		fp2_add(c[0][0], u0[0], u4[1]);
		fp2_add(c[1][1], u0[1], u1[0]);
	} RLC_CATCH_ANY {
		RLC_THROW(ERR_CAUGHT);
	} RLC_FINALLY {
		fp4_free(t0);
...
/////////////////////////////////////////////////////// Copy of basic fp12_sqr_cyc version
void fp12_sqr_cyc_lazyr(fp12_t c, fp12_t a) {
	fp2_t t0, t1, t2, t3, t4, t5, t6;
...
/////////////////////////////////////////////////////// Copy of basic fp12_sqr_pck version
void fp12_sqr_pck_lazyr(fp12_t c, fp12_t a) {
	fp2_t t0, t1, t2, t3, t4, t5, t6;
