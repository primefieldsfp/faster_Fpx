...
void fp2sqr_mont_old(const f2elm_t a, f2elm_t c)
...
{     
    ...
    fp2mul_c1_mont(a[0], b[0], t1);                   // c1 = a0*b1 + a1*b0 
    fp2mul_c0_mont(a[0], b[0], c[0]);                 // c0 = a0*b0 - a1*b1
    fpcopy(t1, c[1]);
}


void fp2mul_mont_old(const f2elm_t a, const f2elm_t b, f2elm_t c)
