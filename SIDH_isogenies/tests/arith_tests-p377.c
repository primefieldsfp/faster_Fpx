...
{ // Tests for the quadratic extension field arithmetic
    ...
    // (Unified) multiplication over GF(p377^2)                    ////////////// TEMPORARY
    passed = 1;
    for (n=0; n<TEST_LOOPS; n++)
    {    
        fp2random377_test((digit_t*)a); fp2random377_test((digit_t*)b); 
              
        to_fp2mont(a, ma); to_fp2mont(b, mb);
        fp2mul377_mont_old(ma, mb, md);                                                      
        fp2mul377_mont(ma, mb, me);                                                    
        from_fp2mont(md, d);
        from_fp2mont(me, e);
        if (compare_words((digit_t*)d, (digit_t*)e, 2*NWORDS_FIELD)!=0) { passed=0; break; }
    }
    ...
    // (Unified) squaring over GF(p377^2)                  ////////////// TEMPORARY
    passed = 1;
    for (n=0; n<TEST_LOOPS; n++)
    {
        fp2random377_test((digit_t*)a);
        ...
        to_fp2mont(a, ma);
        fp2sqr377_mont_old(ma, mb);                                 // b = a^2 
        fp2sqr377_mont(ma, mc);
        from_fp2mont(mb, b);               
        from_fp2mont(mc, c);               
        if (compare_words((digit_t*)b, (digit_t*)c, 2*NWORDS_FIELD)!=0) { passed=0; break; }
    }
